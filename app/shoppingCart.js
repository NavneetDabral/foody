    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item._id != null && item.cuisine != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item._id, item.cuisine, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (_id,cuisine, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(_id,cuisine, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.getTotalPrice = function (_id) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (_id== null || item._id== _id) {
...
// get the total price for all items currently in the cart
shoppingCart.prototype.getTotalCount = function (_id) {
    var count = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (_id == null || item._id == _id) {
...
// items in the cart
//
function cartItem(_id, cuisine, price, quantity) {
    this._id = _id;
